...
export function Sidebar() {
  ...
  const ROLE_COLOR: Record<string, string> = {
    ...
    VERIFIER: 'text-purple-400 bg-purple-900/20',
    POLICE_OFFICER: 'text-green-400 bg-green-900/20',
    TOW_OPERATOR: 'text-amber-400 bg-amber-900/20',
    CITIZEN: 'text-slate-300 bg-slate-800',
    VEHICLE_OWNER: 'text-cyan-400 bg-cyan-900/20',
  };
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 px-4 py-5 border-b border-slate-800">
        ...
        <div>
          <h1 className="text-base font-black text-slate-100">ParkSight<span className="text-blue-400"> AI</span></h1>
        ...
        <div className="px-4 py-3 border-b border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <span className={`text-xs px-1.5 py-0.5 rounded-md font-medium ${ROLE_COLOR[user.role] || 'text-slate-400'}`}>
                {user.role}
              </span>
            </div>
          </div>
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-4 space-y-5">
        {groupedItems.map(({ group, items }) => (
          <div key={group}>
            <div className="px-3 text-xs font-semibold text-slate-500 uppercase tracking-widest mb-2">{group}</div>
      ...
      <div className="border-t border-slate-800 px-4 py-3 text-xs text-slate-600">ParkSight AI v2.0 © 2025</div>
    </div>
  );
  ...
  return (
    <>
      {/* Desktop */}
      <aside className="hidden md:flex w-60 flex-col border-r border-slate-800 bg-slate-950 shrink-0">
        {sidebarContent}
      </aside>
      ...
      <div className="md:hidden">
        <button
          ...
          className="fixed left-4 top-4 z-50 p-2 bg-slate-900 border border-slate-700 rounded-xl"
        >
        ...
        <aside className={cn(
          'fixed inset-y-0 left-0 z-40 w-60 border-r border-slate-800 bg-slate-950 transition-transform duration-200',
